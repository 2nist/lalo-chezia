/**
 * TimeDrawer - Right-side vertical drawer for time cards
 */
...
export default function TimeDrawer({ 
  ...
  const timeFacetOptions = ['groove', 'cadence', 'rhythmic', 'tempo', 'signature'] as const
  ...
  const matchesFacet = (card: IdeaCardData, facet: string) => {
    ...
    const tempoMatch = facet === 'tempo' && !!card.tempoModifier
    return Boolean(tagMatch || labelMatch || tempoMatch)
  }

  const filteredCards = useMemo(() => {
    let filtered = cards.filter(c => c.type === 'time')
  ...
  return (
    <>
      <div className="h-full flex flex-col bg-card border-l border-border w-40">
        {/* Header */}
          ...
          <div className="flex items-center gap-2 text-[10px]">
            <label className="inline-flex items-center gap-1 text-amber-200">
              <input
                ...
                aria-label="Show global time cards"
              />
              <span>Global</span>
            </label>
            <label className="inline-flex items-center gap-1 text-blue-200">
              <input
                type="checkbox"
                checked={showLocal}
                onChange={(e) => setShowLocal(e.target.checked)}
                className="h-3 w-3 rounded border-border bg-background"
                aria-label="Show local time cards"
              />
              <span>Local</span>
            </label>
          </div>
          ...
          <div className="flex items-center justify-between gap-1">
              ...
              return (
                ...
                >
                  <input
                    ...
                    aria-label={`Filter time cards by ${facet}`}
                  />
            ...
            {activeFacets.length > 0 && (
              <button
                ...
                aria-label="Clear time facet filters"
              >
                Clear
              </button>
            )}
          ...
          {showLocal && localCards.length > 0 && (
            <div className="space-y-1">
              <button
                ...
                className="w-full flex items-center justify-between text-[10px] font-semibold text-blue-200 px-2 py-1.5 rounded bg-blue-500/10 border border-blue-500/20 uppercase tracking-wide"
              >
